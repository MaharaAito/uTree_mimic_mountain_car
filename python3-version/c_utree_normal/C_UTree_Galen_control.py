...
class CUTree:
    ...
    def sweepRecursive(self, node, gamma):
        ...
        if node.nodeType == NodeLeaf:
            for action, reward in enumerate(node.rewards):
                ...
                exp = 0
                for node_to, t in list(node.transitions[action].items()):
            ...
            for record in reader:
                ...
                if not record[4]:  # qValues determines NodeSplit or NodeLeaf
                    ...
                    node = UNode(int(record[0]), NodeLeaf, self.nodes[int(record[3])] if record[3] else None,
                                 self.n_actions, self.nodes[int(record[3])].depth + 1 if record[3] else 1)
                    node.qValues_home = np.array(list(map(float, record[4][1:-1].split())))
                    node.qValues_away = np.array(list(map(float, record[5][1:-1].split())))
                if node.parent:
        ...
        if node.nodeType == NodeSplit:
            with open(file_directory, 'a') as f:
                print(blank + "idx={}, dis={}, par={}".format(node.idx,
                                                                    node.distinction.dimension,
                                                                    node.parent.idx if node.parent else None), file=f)

            for child in node.children:
                self.print_tree_recursive(blank + " ", child, file_directory)
        else:
            with open(file_directory, 'a') as f:
                print(blank + 'idx={}'.format(node.idx), file=f)
                print(blank + blank + 'r={}'.format(node.rewards), file=f)
                print(blank + blank + 't={}'.format(node.transitions), file=f)
                print(blank + blank + 'q={}'.format(['{0:.2f}'.format(q) for q in node.qValues]), file=f)
                print(blank + blank + 'par={}'.format(node.parent.idx if node.parent else None), file=f)
    ...
    def unsplit(self, node):
        ...
        if node.nodeType == NodeSplit:
            ...
            # Re-build model for all nodes
            for i, n in list(self.nodes.items()):
                if n.nodeType == NodeLeaf:
                    self.modelFromInstances(n)
...
class Instance:
    ...
    def __init__(self, timestep, currentObs, action, nextObs, reward):
        ...
        self.nextObs = list(map(float, nextObs))  # record the state data
        self.currentObs = list(map(float, currentObs))  # record the state data
        self.reward = reward
