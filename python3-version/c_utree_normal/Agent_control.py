...
import scipy as sp
import scipy.stats
# from . import C_UTree_Galen_control
...
class CUTreeAgent:
    ...
    def executePolicy(self, currentObs, epsilon):
        ...
        test = random.random()
        if test < epsilon:
            return random.choice(list(range(len(self.problem.actions))))
        return self.utree.getBestAction(currentObs, list(self.problem.actions.values()))
