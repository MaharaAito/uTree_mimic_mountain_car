        ...
        with open(filename, 'rb') as (csvfile):
            ...
            for record in reader:
                ...
                if not record[4]:
                    ...
                    node.qValues_end = np.array(list(map(float, record[6][1:-1].split())))
                if node.parent:
        ...
        if node.nodeType == NodeLeaf:
            for action, reward in enumerate(node.rewards_home):
                ...
                if c == 0:
                    continue
                exp = 0
                for node_to, t_h in list(node.transitions_home_home[action].items()):
            ...
            for action, reward in enumerate(node.rewards_away):
                ...
                if c == 0:
                    continue
                exp = 0
                for node_to, t_h in list(node.transitions_away_home[action].items()):
...
class Instance:
    ...
    def __init__(self, timestep, currentObs, action, nextObs, reward, qValue):
        ...
        self.nextObs = list(map(float, nextObs))
        self.currentObs = list(map(float, currentObs))
        self.reward = reward
        self.qValue = qValue
